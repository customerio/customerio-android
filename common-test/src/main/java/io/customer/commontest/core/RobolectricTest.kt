...
abstract class RobolectricTest : UnitTest() {
    ...
    init {
        ...
        every { contextMock.packageName } returns testPackageName
        every { contextMock.packageManager } returns packageManagerMock
        every { contextMock.checkCallingOrSelfPermission(Manifest.permission.ACCESS_NETWORK_STATE) } returns PackageManager.PERMISSION_GRANTED
