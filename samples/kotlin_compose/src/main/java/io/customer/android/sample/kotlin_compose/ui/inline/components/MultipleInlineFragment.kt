...
class MultipleInlineFragment : Fragment() {
    ...
    private fun setupContent() {
        binding.fetchMessagesButton.setOnClickListener {
            ...
            findAndShowMessage(
                binding.stickHeaderInAppMessage,
                InlineUtils.ELEMENT_ID_STICKY_HEADER
            )
            findAndShowMessage(binding.inlineInAppMessage, InlineUtils.ELEMENT_ID_INLINE)
            findAndShowMessage(binding.belowFoldInAppMessage, InlineUtils.ELEMENT_ID_BELOW_FOLD)
        }
    }
