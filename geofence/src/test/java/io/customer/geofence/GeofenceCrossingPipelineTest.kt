...
class GeofenceCrossingPipelineTest : RobolectricTest() {
    ...
    /**
     * The real pipeline, resolved from the graph the overrides above shaped.
     *
     * These decisions used to be reachable only through `GeofenceBroadcastReceiver`, an OS class
     * with no constructor to inject — the tests had to call a `@VisibleForTesting` method that
     * still spoke GMS transition codes. They now speak the SDK's own vocabulary, which is the
     * point: a `GeofenceCrossingTransition.EXIT` here means the same thing whether GMS, replay, or
     * a future platform produced it.
     */
    private val pipeline get() = SDKComponent.android().geofenceCrossingPipeline

    /**
     * Feeds one crossing through the pipeline.
     *
     * [rawTransitionCode] only matters for the unsupported case, where the record names the code
     * the OS sent. Translating GMS codes into [GeofenceCrossingTransition] is the receiver's job
     * and is pinned by `GeofenceBroadcastReceiverTest`, not duplicated here.
     */
