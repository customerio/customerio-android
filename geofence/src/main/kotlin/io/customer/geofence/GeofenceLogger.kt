...
internal class GeofenceLogger(private val logger: Logger) {
    ...
    /**
     * The OS call underneath a reset. An observation: `module.reset` is the decision a scenario
     * grades, and this is the mechanism beneath it — the same relationship `registration.added`
     * has to `registration.applied`. iOS has no counterpart record at all.
     */
    ...
    /**
     * The location permission the user has granted, reported when it changes.
     *
     * Android has no permission observer — only a poll — so this is read on foreground entry, which
     * is when a user returning from Settings would have changed it. The two records below fire on
     * denial paths only, so before this a granted permission produced nothing at all and both
     * 2026-09-09 drive captures show zero `permission.changed` against iOS's three.
     *
     * `perm` uses iOS's vocabulary (`denied` / `when_in_use` / `always`) rather than Android's
     * permission constants, so one parser reads both platforms.
     */
    ...
    /**
     * Sign-out observed; the wipe is about to be attempted.
     *
     * Deliberately `ev=info`, not `module.reset`: that key is pinned to `out` and now means the
     * outcome — cleared, or declined to clear. A record for the *intent* under the same key would
     * make the key mean two different things depending on which site fired, which is the drift the
     * per-`ev` pin exists to prevent. `identity.changed ok=false` is already the input that says a
     * sign-out happened, so this record is for a human reading a capture, nothing more.
     */
    ...
    /**
     * Sign-out's outward half: monitoring stopped and user-scoped state cleared.
     *
     * An output, and the same key iOS asserts. "Logout clears geofences" is a locked
     * cross-platform contract; before this the two platforms recorded it under different keys —
     * iOS on `module.reset`, Android on `registration.cleared` — so one scenario format could not
     * grade both.
     */
     ...
     *
     * Asserted too. A regression that wipes an active user's geofences is the more damaging
    ...
    /**
     * A position arrived from outside: the Location module published a fresh fix.
     *
     * Frozen as *arrival*, not *read*. The anchor reads in `ModuleGeofence.refreshAnchor` consult a
     * position the SDK already holds; logging those would let a replay inject fixes the OS never
     * delivered, and inflate the count that says how often the device actually reported itself.
     *
     * No `acc` or `age`: [io.customer.sdk.communication.Event.LocationAcquired] carries coordinates
     * only. Widening a core event to enrich a diagnostic would be a contract change, not a logging
     * one — and nothing on this platform reads accuracy, so replay loses no fidelity.
     */
